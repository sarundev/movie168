...
import { createCreditTopUpAction } from "../actions/movie-actions";
          ...
          <div key={lb} className="flex items-center">
            <div className="flex flex-col items-center gap-1.5">
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-[11px] font-black transition-all duration-300"
                style={{
                  background: done ? "#c9a835" : active ? "rgba(201,168,53,0.13)" : "rgba(255,255,255,0.04)",
                  border:     done || active ? "2px solid #c9a835" : "2px solid rgba(255,255,255,0.08)",
                  color:      done ? "#0d0d12" : active ? "#c9a835" : "#444",
                  boxShadow:  active ? "0 0 12px rgba(201,168,53,0.4)" : "none",
                }}>
                {done
              ...
              <span className="text-[9px] font-semibold whitespace-nowrap hidden sm:block"
                style={{ color: active ? "#c9a835" : done ? "#555" : "#333" }}>{lb}</span>
            </div>
            {i < labels.length - 1 && (
...
export default function DepositPage() {
  ...
  const selPack = CREDIT_PACKS.find(p => p.id === pack)!;

  async function handlePay() {
    setLoading(true); setError(null);
    const res = await createCreditTopUpAction(selPack.credits);
    setLoading(false);
    if (res.ok && res.data) {
    ...
    <div>
        ...
        {CREDIT_PACKS.map(p => {
          ...
          return (
            <button key={p.id} onClick={() => setPack(p.id)}
              ...
              <p className="text-2xl font-black mt-1" style={{ color: accent }}>{p.credits}</p>
              <p className="text-[10px]" style={{ color: sel ? accent : "#555" }}>Credits</p>
      ...
      <div className="rounded-2xl p-4 mb-5" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)" }}>
        <div className="flex justify-between text-xs mb-2">
          <span style={{ color: "#555" }}>Credits</span>
          <span style={{ color: "#c9a835" }} className="font-bold">{selPack.credits} Credits</span>
        </div>
        <div className="flex justify-between text-xs mb-2">
      ...
      <PrimaryBtn onClick={handlePay} loading={loading}>
        {loading ? "កំពុងបង្កើត QR..." : `ស្កេន KHQR · ${selPack.usd} →`}
      </PrimaryBtn>
    </div>
  );
    ...
    <div className="flex flex-col items-center py-4 gap-4">
      ...
      <div className="text-center">
        ...
        <p className="text-sm" style={{ color: "#555" }}>
          <span className="font-bold" style={{ color: "#c9a835" }}>{selPack.credits} Credits</span> ត្រូវបានបន្ថែមទៅ Wallet
        </p>
      </div>

      <div className="flex gap-3 w-full mt-2">
        <button onClick={() => { setStep(1); setModal(null); }}
  ...
  const labels  = ["ជ្រើស Credits", "ជោគជ័យ"];
  const titles  = ["ជ្រើសសំណុំ Credits", "ការទូទាត់បានជោគជ័យ"];
  const content = [Step1, Step2][step - 1];
    ...
    <div className="min-h-screen" style={{ background: "#0d0d12" }}>
      ...
      <div className="pt-24 pb-2 px-4 sm:px-6 max-w-2xl mx-auto">
        <div className="flex items-center gap-1.5 text-[11px] mb-3" style={{ color: "#444" }}>
          ...
          <span>/</span>
          <span style={{ color: "#c9a835" }}>ទិញ Credits</span>
        </div>
        <h1 className="text-2xl font-black" style={{ color: "#f0f0f0" }}>ទិញ Credits</h1>
        <p className="text-sm mt-1" style={{ color: "#555" }}>Credits ត្រូវបានប្រើសម្រាប់ទិញរឿងដែលទាមទារការទូទាត់</p>
      </div>
      ...
      {modal && (
        <KhqrPayModal
          ...
          amount={modal.amount}
          label={`${selPack.credits} Credits Top-Up`}
          onSuccess={() => { setModal(null); setStep(2); }}
