export default function Footer() {
  return (
    <footer
      className="mt-12 px-4 sm:px-6 lg:px-12 pt-10 pb-6"
      style={{ borderTop: "1px solid rgba(201,168,53,0.15)" }}
    >
      {/* Logo + tagline */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-10">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            <div
              className="w-8 h-8 flex items-center justify-center rounded text-xs font-black"
              style={{
                background: "linear-gradient(135deg, #c9a835, #8a6e1a)",
                color: "#0d0d12",
              }}
            >
              168
            </div>
            <span
              className="text-xl font-black tracking-widest"
              style={{ color: "#c9a835" }}
            >
              168NET
            </span>
          </div>
          <p className="text-xs" style={{ color: "#666" }}>
            Stream the world&apos;s best movies and TV shows — anytime, anywhere.
          </p>
        </div>

        {/* Social icons */}
        <div className="flex items-center gap-2">
          {[
            {
              label: "Twitter",
              path: "M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.746l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z",
            },
            {
              label: "Instagram",
              path: "M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z",
            },
            {
              label: "Facebook",
              path: "M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z",
            },
          ].map(({ label, path }) => (
            <a
              key={label}
              href="#"
              className="footer-social-icon w-8 h-8 flex items-center justify-center rounded transition-all"
              aria-label={label}
            >
              <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor">
                <path d={path} />
              </svg>
            </a>
          ))}
        </div>
      </div>

      {/* Divider */}
      <div  
        className="mb-8 h-px"
        style={{ background: "linear-gradient(to right, rgba(201,168,53,0.3), rgba(201,168,53,0.05), transparent)" }}
      />

      {/* Links grid */}
      {/* <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 mb-8">
        {columns.map((col) => (
          <div key={col.title}>
            <h4
              className="text-xs font-bold uppercase tracking-widest mb-3"
              style={{ color: "#c9a835" }}
            >
              {col.title}
            </h4>
            <ul className="space-y-2">
              {col.links.map((link) => (
                <li key={link}>
                  <a
                    href="#"
                    className="text-sm transition-colors"
                    style={{ color: "#666" }}
                    onMouseEnter={(e) => (e.currentTarget.style.color = "#c9a835")}
                    onMouseLeave={(e) => (e.currentTarget.style.color = "#666")}
                  >
                    {link}
                  </a>
                </li>
              ))}
            </ul>
          </div>
        ))}
      </div> */}

      {/* Bottom */}
      {/* <div
        className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pt-5"
        style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}
      >
        <p className="text-xs" style={{ color: "#444" }}>
          &copy; 2025 168NET. All rights reserved. For entertainment purposes only.
        </p>
        <div className="flex items-center gap-4">
          {["Privacy", "Terms", "Cookies"].map((item) => (
            <a
              key={item}
              href="#"
              className="text-xs transition-colors"
              style={{ color: "#444" }}
              onMouseEnter={(e) => (e.currentTarget.style.color = "#888")}
              onMouseLeave={(e) => (e.currentTarget.style.color = "#444")}
            >
              {item}
            </a>
          ))}
        </div>
      </div> */}
    </footer>
  );
}
